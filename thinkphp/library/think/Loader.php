...
class Loader
{
    ...
    {
        var_dump(static::$prefixDirsPsr4);
    }
    ...
    public static function register($autoload = '')
    {
        ...
        // 上面这两步骤 就是往 self 的prefixlengthpsr4 和 prefixdirspsr4 中写入了4个命名空间
     ...
     //   self::addAutoLoadDir($rootPath . 'cy');
    ...
    public static function factory($name, $namespace = '', ...$args)
    {
        ...
        if (class_exists($class)) {
            return Container::getInstance()->invokeClass($class, $args); // 虽然可以通过反射实现类，但必须传入必要的参数哈。那些类参数可以依赖注入，非类的参数，那就不行喽，非类的采纳数一定要传入
        } else {
